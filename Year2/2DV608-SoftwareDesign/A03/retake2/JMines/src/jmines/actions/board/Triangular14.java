...
/**
 * The class representing the action used when the user click the Parquet menu
 * item.
 *
 * @author Zleurtor
 */
public class Triangular14 extends JMinesAction {
     ...
     */
    private static final long serialVersionUID = 628826551531928027L;
    ...
    //==========================================================================
    /**
     * Construct a new Triangle14 action.
     *
     * @param name The name of the menu item.
     * @param mainFrame The main frame of the application.
     */
    public Triangular14(final String name, final MainFrame mainFrame) {
        super(name, mainFrame);

        setStatusText(Configuration.getInstance().getText(Configuration.KEY_STATUSTEXT_BOARD_TRIANGULAR_14));
    }
    ...
    public final void actionPerformed(final ActionEvent evt) {
        try {
            getMainPanel().getGamePanel().getGameBoard().setTilesShape(GameBoard.SHAPE_TRIANGULAR_14);

            int width = getMainPanel().getGamePanel().getGameBoard().getWidth();
            int height = getMainPanel().getGamePanel().getGameBoard().getHeight();
            int maxMines = GameBoard.getMaxMines(GameBoard.SHAPE_TRIANGULAR_14, width, height);
        ...
        Configuration.getInstance().putRealTimeconfiguration(Configuration.KEY_USER_SHAPE, Configuration.SHAPE_TRIANGULAR_14);
